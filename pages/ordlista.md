...
Den mobila applikationen där en användare förvarar, hanterar och väljer att dela sina digitala intyg (t.ex.
PID) med olika förlitande parter. :
...
Det grundläggande identitetsintyget (t.ex.
namn, personnummer, födelsedatum) som utfärdas av den statliga PID-utfärdaren till användarens plånbok.
:
...
Den testplattform som tillhandahålls av Digg där utvecklare och förlitande parter kan bygga och testa interoperabilitet för sina tjänster i en säker och isolerad miljö. Även kallad "testplattformen".
...
Det tillåter selektivt utlämnande, vilket innebär att användaren kan välja att enbart dela specifika uppgifter (t.ex.
bekräfta att man är över 18 år) utan att avslöja hela sitt personnummer eller namn.
:
...
Den säkra hårdvarubaserade eller mjukvarubaserade kryptografiska modul i användarens enhet (t.ex.
Secure Element) som skyddar plånbokens nycklar mot obehörig åtkomst och kopiering.
:
