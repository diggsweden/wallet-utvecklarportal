...
- Systemberoenden för att kompilera eventuella gems (t.ex.
  `build-essential`, `gcc`, `make` på Ubuntu/Debian).
...
> \[!IMPORTANT] För att detta ska fungera och för att undvika det inbyggda, felaktiga GitHub Pages-bygget (som inte får med alla resurser som diagram och skapar ett race condition) måste GitHub Pages vara konfigurerat för att använda **GitHub Actions** som källa.
> Det ställs in under **Settings** -> **Pages** -> **Build and deployment** -> **Source:
> GitHub Actions**.
...
4. Välj den branch du vill testa (t.ex.
   `feat/min-ändring`) under **Use workflow from**.
5. Klicka på den gröna **Run workflow**-knappen.
6. Det tar ungefär 1-2 minuter innan ändringarna visas på <https://diggsweden.github.io/wallet-utvecklarportal/>.
