...
# Quality checks and automation for Wallet Utvecklarportal
...
default:
    @printf "{{CYAN_BOLD}} Wallet Utvecklarportal{{NC}}\n\n"
...
check-tools: _ensure-devtools
    @{{devtools_dir}}/scripts/check-tools.sh --check-devtools mise git just rumdl yamlfmt actionlint gitleaks gommitlint shellcheck shfmt reuse
